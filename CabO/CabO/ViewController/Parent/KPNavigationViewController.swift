...
//        swipeInteractionController.wireToViewController(viewController)
    ...
    func navigationController(_ navigationController: UINavigationController, didShow viewController: UIViewController, animated: Bool) {
        if viewController is NTabBarVC {
            self.interactivePopGestureRecognizer!.isEnabled = false
        }
        else if let vc = viewController as? CreateRideVC {
            self.interactivePopGestureRecognizer!.isEnabled = vc.createRideStep == .step1
        }
        else if let vc = viewController as? SignUpVC {
            self.interactivePopGestureRecognizer!.isEnabled = vc.screenType == .profile
        }
        else if let _ = viewController as? VerifyOtpVC {
            self.interactivePopGestureRecognizer!.isEnabled = false
        }
        else if let _ = viewController as? ResetPasswordVC {
            self.interactivePopGestureRecognizer!.isEnabled = false
        }
        else {
